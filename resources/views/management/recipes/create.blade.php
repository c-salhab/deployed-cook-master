<x-management-layout>
    ...
    <div class="space-y-8 divide-y divide-gray-200 w-1/2 mt-10">
        <form method="POST" action="{{ route('management.recipes.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="sm:col-span-6 pt-5">
                ...
                <div class="mt-1">
                    <textarea id="ingredients" rows="3"  class="shadow-sm focus:ring-indigo-500 appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" name="ingredients"></textarea>
                </div>
            </div>
            @error('ingredients')
            <div class="text-sm text-red-400">{{ $message }}</div>
            ...
            <div class="sm:col-span-6 pt-5">
                ...
                <div class="mt-1">
                    <textarea id="steps" rows="3"  class="shadow-sm focus:ring-indigo-500 appearance-none bg-white border border-gray-400 rounded-md py-2 px-3 text-base leading-normal transition duration-150 ease-in-out focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md" name="steps"></textarea>
                </div>
            </div>
            @error('steps')
            <div class="text-sm text-red-400">{{ $message }}</div>
            ...
            <div class="mt-6 p-4">
            <button type="submit" class="px-4 py-2 bg-indigo-500 hover:bg-indigo-700 rounded-lg text-white">Create New Recipe</button>
            </div>
        </form>
    </div>
